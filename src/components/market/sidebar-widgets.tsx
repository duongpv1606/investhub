...
export function NewsSidebar() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
      ...
      <div style={panel}>
        {/* Header */}
        <div style={{ padding: "12px 14px", borderBottom: "1px solid var(--card-border)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", gap: "4px" }}>
            {([["breaking", "breaking", "Tin nóng"], ["trending", "trending", "Trending"]] as const).map(([k, , l]) => (
              <button key={k} onClick={() => setTab(k as any)} style={{
                ...
                letterSpacing: "0.2px",
                background: tab === k ? "var(--primary-muted)" : "transparent",
            ...
            return (
              <a key={a.id || i}
                ...
                style={{
                  ...
                  padding: "11px 14px",
                  borderBottom: i < 7 ? "1px solid var(--card-border)" : "none",
                  textDecoration: "none",
      ...
      <div style={{ ...panel, padding: "15px" }}>
        <div style={{ fontSize: "10px", fontFamily: "'JetBrains Mono',monospace", textTransform: "uppercase", letterSpacing: "1px", color: "var(--text-dim)", marginBottom: "14px" }}>
          Market Pulse
        </div>
        {[
          ...
          <div key={p.l} style={{ marginBottom: "12px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: "5px" }}>
              ...
              <div style={{ display: "flex", alignItems: "baseline", gap: "5px" }}>
                ...
                <span style={{ fontSize: "10px", color: "var(--text-dim)" }}>{p.desc}</span>
              </div>
            </div>
            <div style={{ height: "4px", background: "var(--card-hover)", borderRadius: "2px", overflow: "hidden" }}>
      ...
      {/* Top Movers */}
      <div style={{ ...panel, padding: "15px" }}>
        <div style={{ fontSize: "10px", fontFamily: "'JetBrains Mono',monospace", textTransform: "uppercase", letterSpacing: "1px", color: "var(--text-dim)", marginBottom: "12px" }}>
          Top tăng mạnh
        </div>
          ...
          <div key={stock.symbol} style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "9px" }}>
              ...
              <div>
                ...
                <div style={{ fontSize: "11px", color: "var(--text-dim)", fontFamily: "'JetBrains Mono',monospace", fontFeatureSettings: '"tnum"' }}>{(stock.price / 1000).toFixed(1)}k</div>
              </div>
            </div>
    ...
    <div style={{ ...panel, padding: "15px" }}>
      ...
      {sorted.map((stock, i) => (
        <div key={stock.symbol} style={{
          display: "flex", alignItems: "center", justifyContent: "space-between",
          padding: "8px 0",
          borderBottom: i < sorted.length - 1 ? "1px solid var(--card-border)" : "none",
        }}>
          <div style={{ display: "flex", alignItems: "center", gap: "9px" }}>
            <div style={{
              width: "30px", height: "30px", borderRadius: "8px",
            ...
            <div>
              <div className="table-sym">{stock.symbol}</div>
              <div style={{ fontSize: "11px", color: "var(--text-dim)", fontFamily: "'JetBrains Mono',monospace" }}>{(stock.price / 1000).toFixed(1)}k</div>
            </div>
          </div>
          <div style={{ textAlign: "right" }}>
            <div className={stock.changePercent >= 0 ? "pct-up" : "pct-down"}>
      ...
      {sorted.length === 0 && (
        <p style={{ fontSize: "12px", color: "var(--text-dim)", textAlign: "center", padding: "8px 0" }}>Chưa có dữ liệu</p>
      )}
    </div>
  );
}
...
export function TelegramCTA() {
  return (
    <div id="telegram" style={{
      ...
      <Send size={20} color="var(--primary)" style={{ margin: "0 auto 10px" }} />
      <div style={{ fontSize: "14px", fontWeight: 600, color: "var(--text)", marginBottom: "6px", letterSpacing: "-0.1px" }}>
        Tín hiệu giao dịch
      </div>
      ...
      <a href="https://t.me/markethub_vn" target="_blank" rel="noopener noreferrer"
        style={{
          ...
          color: "var(--primary)", fontSize: "12.5px", fontWeight: 600, textDecoration: "none",
          letterSpacing: "0.1px",
        }}>
        Tham gia miễn phí →
      </a>
